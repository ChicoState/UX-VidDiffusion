...
During the first and second sprint of our project, we conducted a heuristic evaluation of our competitors and a cognitive walkthrough of our prototype. We used the results of these evaluations to improve design for our prototype and create a set of tasks for our usability tests. 

The results of the heuristic evaluation on a competitor (runway.ml) hightlighted the emphasis of visibility of system status, error prevention, consistency and standards. These concepts were applied to our prototype to improve the user experience. For example, the use of a progress bar to indicate the status of dependency installation during install is used. Additionally, sliders are used to support the use of variable intensity for the application of stable diffusion.

The results of the cognitive walkthroughs on our prototype highlighted the need for a more recognizable design. It was noted by the evaluators that the prototype was a bit difficult to navigate due to small buttons. This was addressed by increasing the visilibity of buttons, navigation buttons were also added. Additionally, the addition of a progress bar to indicate the status of video processing was added to address the issue of successful completion of video processing.
...
Our created prototypes do not actually represent the functionality of all features. By this we mean we had to simulate parts of the tasks as completed by the user because adobe XD didnt support them. For example, subjects could not actually select a video to upload to the software. Given our project, the major differences between scenarios and tasks is how the user is able to use the finer detailed featurer. As a user, being able to type their input into a functioning text box and recieving an output would greatly proffit the experiment. Having lost out on such control of features, severely reduced the amount of data we were able to collect on each specific feature and its functionality. We were not able to truely simulate if a user is able to complete the task with a working and complete version.
